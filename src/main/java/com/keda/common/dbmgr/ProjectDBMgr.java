...
public class ProjectDBMgr {
    ...
    private ProjectDBMgr() {
        ...
        username.put("project_001", "root");
        username.put("project_002", "root");
        username.put("project_003", "root");

        password.put("project_001", "admin");
        password.put("project_002", "admin");
        password.put("project_003", "admin");
    }
    ...
    public String getUsername(String projectCode) {
        if (username.containsKey(projectCode)) {
            return username.get(projectCode);
        }
        return "";
    }

    public String getPassword(String projectCode) {
        if (password.containsKey(projectCode)) {
            return password.get(projectCode);
        }
        return "";
    }
}
